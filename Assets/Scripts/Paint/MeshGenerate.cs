...
public class MeshGenerate : MonoBehaviour
{
    ...
    public void GenerateMesh(List<Vector3> PointPos,List<Vector3> GetPointPos, int Getwidth,int count)
    {
        ...
        Transform[] bones = new Transform[PointPos.Count / 5];
        Matrix4x4[] bindPoses = new Matrix4x4[PointPos.Count / 5];
        if (PointPos.Count % 5 != 0)
        {
            bones = new Transform[PointPos.Count / 5 + 1];
            bindPoses = new Matrix4x4[PointPos.Count / 5 + 1];
        }
        ...
        for (int i = 0, n = 0; i < bones.Length; i++, n += 5)
        {
            if (i == 0)
            {
        ...
        rend.probeAnchor = GameObject.Find("Root/J_Bip_C_Hips/J_Bip_C_Spine/J_Bip_C_Chest/J_Bip_C_UpperChest/J_Bip_C_Neck/J_Bip_C_Head").transform;
